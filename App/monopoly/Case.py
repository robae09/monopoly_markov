...
class Case:
    ...
    """
        ...
        @param prix pour acheter la case (-1 pour une case qui ne peut pas être achetée)
            (Par défaut: 0)
        @param gainVide le gain que l'on fait sans avoir rien acheté sur cette case (pas d'hotel ect)
        @param gainHotel le gain que l'on fait en ayant acheté un hotel sur cette case
        @param prixAppartement le prix pour construire un appartement sur la propriété
        @param prixHotel le prix pour construire un hotel sur la propriété
        @param vendre permet de savoir si une case est a vendre par la banque où pas (si elle peut 
            être achetée par un joueur)
            (Par défaut: True)
    """
    def __init__(self, nom, position, couleur = "black", prix = 0, gainVide=0, gainHotel=0, \
            prixAppartement=0, prixHotel=0, vendre=True):

        self._nom = nom
        self._position = position
        self._couleur = couleur
        self._prix = prix
        self._gainVide = gainVide
        self._gainHotel = gainHotel
        self._prixAppartement = prixAppartement
        self._prixHotel = prixHotel
        self._nbrDeDouble = 0
    ...
    """
        Permet de récupérer la couleur de la case sous forme HTML

        @return la couleur de la case (en HTML)
    """
    def getHtmlCouleur(self):
        return self.__getHtmlColor(self.getCouleur())
    ...
    """
        Permet de récupérer le prix de la case si l'on y construite en plus un hotel (donc avant ça
            4 appartements)

        @return le prix de la case et de la construction
    """
    def getPrixHotel(self):
        return self.getPrix() + 4*self._prixAppartement + self._prixHotel

    """
        Permet de récupérer le gain de la case lorsque celle-ci est vide

        @return le gain de la case
    """
    def getGainVide(self):
        return self._gainVide

    """
        Permet de récupérer le gain de la case lorsque celle-ci comporte unhotel

        @return le gain de la case
    """
    def getGainHotel(self):
        return self._gainHotel


    """
        Permet de récupérer le ratio entre le prix d'achat de la case et le gain lorsque la case
        est vide

        @return le ratio
    """
    def getRatioPrixGainVide(self):
        result = 0
        if(self.getGainVide() > 0):
            result = self.getPrix()/self.getGainVide()
        return result
    ...
    """
        ...
        @return le ratio
    """
    def getRatioPrixGainHotel(self):
        result = 0
        if(self.getGainHotel() > 0):
            result = (self.getPrixHotel()) / self.getGainHotel()
        return result
    ...
    """
        Permet de récupérer le nombre de fois que l'on doit visiter cette case vide (sans hotel, ect) 
        pour être rentable

        @return le nombre de fois qu'il faut visiter la case pour être rentable
    """
    def getRentabiliteVide(self):
        return getPrix()/getGainVide()
